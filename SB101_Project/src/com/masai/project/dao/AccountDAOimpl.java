...
//*****************************************************************************************
	     ...
	     public List<AccountDTO> viewAllInoperativeAccounts() throws SomethingWentWrongException, NoRecordFoundException {
	    	 ...
	    	 List<AccountDTO> list = new ArrayList<>();
	            ...
	            try {
	                ...
	                String query = "SELECT account_number, balance, status, customer_id FROM account WHERE status = 'inoperative'";
	                PreparedStatement ps = conn.prepareStatement(query);
	                ResultSet rs = ps.executeQuery();
	               
	                if (DBUtils.isResultSetEmpty(rs)) {
	                    throw new NoRecordFoundException("No inoperative accounts found");
	                }
	                while (rs.next()) {
	                    AccountDTO account = new AccountDTOimpl();
	                    account.setAccountNumber(rs.getInt("account_number"));
	                    account.setBalance(rs.getDouble("balance"));
	                    account.setAccountStatus("INOPERATIVE");
	                    account.setCustomerId(rs.getInt("customer_id"));
	                    list.add(account);
	                }
	            } catch (ClassNotFoundException | SQLException ex) {
	                throw new SomethingWentWrongException("Unable to get inoperative accounts");
	            } finally {
	                try {
	                    DBUtils.closeConnection(conn);
	                } catch (SQLException ex) {
	     ...
	     public List<AccountDTO> viewAllClosedAccounts() throws SomethingWentWrongException, NoRecordFoundException {
	    	    ...
	    	    try {
	    	        conn = DBUtils.getConnectionTodatabase();
	    	        String query = "SELECT account_number, balance, status, customer_id FROM account WHERE status = 'CLOSED'";
	    	        PreparedStatement ps = conn.prepareStatement(query);
	    	        ResultSet rs = ps.executeQuery();
	    	        if (DBUtils.isResultSetEmpty(rs)) {
	    	            throw new NoRecordFoundException("No closed accounts found");
	    	        }
	    	        while (rs.next()) {
	    	            AccountDTO account = new AccountDTOimpl();
	    	            account.setAccountNumber(rs.getInt("account_number"));
	    	        ...
	    	        throw new SomethingWentWrongException("Unable to get closed accounts");
	    	    } finally {
	    	        try {
	    	            DBUtils.closeConnection(conn);
	    	        } catch (SQLException ex) {
